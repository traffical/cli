...
export interface InitResult {
  ...
  sdk_key: { created: boolean; key_prefix?: string; publishable_key_prefix?: string };
  next_steps: string[];
}
...
async function provisionSdkKey(
  ...
  try {
    // Two keys, because the two halves of an app need different credentials and
    // scaffolding only a secret one is how it ends up in a browser bundle. The
    // publishable key is the one the client templates reference; the server key
    // never leaves the backend. Minted together so neither is the easy default.
    const serverKey = await client.createApiKey(org.id, {
      name: `CLI server SDK key — ${project.name}`,
      projectId: project.id,
      kind: "sk",
      scopes: ["sdk:read", "sdk:write"],
    });

    // Non-fatal on its own: a control plane predating publishable keys rejects
    // `kind` outright, and a working server key beats failing the whole init.
    let publishableKey: Awaited<ReturnType<typeof client.createApiKey>> | null = null;
    try {
      publishableKey = await client.createApiKey(org.id, {
        name: `CLI publishable key — ${project.name}`,
        projectId: project.id,
        kind: "pk",
        scopes: ["sdk:read", "sdk:write"],
      });
      ...
      process.stderr.write(
        `⚠ Could not create publishable key: ${err instanceof Error ? err.message : String(err)}\n` +
          "  Client-side code will need one — create it in Settings → API Keys.\n"
      );
    }

    const content = [
      "# Traffical keys — auto-generated by `traffical init`.",
      "# Gitignored by .traffical/.gitignore.",
      "",
      "# Server-side only. Reads the full ruleset; keep it on your backend.",
      `TRAFFICAL_API_KEY=${serverKey.key}`,
      ...(publishableKey
        ? [
            "",
            "# Client-facing applications — browsers and client devices.",
            "# This is the one to expose through PUBLIC_/NEXT_PUBLIC_/VITE_ vars.",
            `TRAFFICAL_PUBLISHABLE_KEY=${publishableKey.key}`,
          ]
        : []),
      "",
    ].join("\n");
    ...
    return {
      created: true,
      keyPrefix: serverKey.apiKey.keyPrefix,
      publishableKeyPrefix: publishableKey?.apiKey.keyPrefix,
      envPath,
    };
  } catch (err) {
...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (!isJson) {
    if (sdk.created) {
      console.log(chalk.green(`✓ Provisioned SDK keys → ${TRAFFICAL_DIR}/.env`));
      console.log(chalk.dim(`  ${sdk.keyPrefix} — server-side only (TRAFFICAL_API_KEY)`));
      if (sdk.publishableKeyPrefix) {
        console.log(
          chalk.dim(`  ${sdk.publishableKeyPrefix} — client-facing (TRAFFICAL_PUBLISHABLE_KEY)`)
        );
      }
    } else if (sdk.envPath) {
  ...
  const result: InitResult = {
    ...
    sdk_key: {
      created: sdk.created,
      key_prefix: sdk.keyPrefix,
      publishable_key_prefix: sdk.publishableKeyPrefix,
    },
