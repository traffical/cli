...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (options.sdkKey !== false) {
    try {
      const newKey = await client.createApiKey(selectedOrg.id, {
        name: `CLI-generated SDK key (${selectedProject.name})`,
        projectId: selectedProject.id,
        scopes: ["sdk:read", "sdk:write"],
      });

      // Write to .traffical/.env
      const envPath = join(projectDir, TRAFFICAL_DIR, ".env");
      const envContent = [
        "# Traffical SDK key (auto-generated by traffical init)",
        "# Use this key in your application for runtime parameter resolution and event tracking.",
        `TRAFFICAL_API_KEY=${newKey.key}`,
        "",
      ].join("\n");
      await writeFile(envPath, envContent, "utf-8");

      // Ensure .traffical/.gitignore contains .env
      await ensureTrafficalGitignore(projectDir);

      sdkKeyResult = {
        created: true,
        keyPrefix: newKey.apiKey.keyPrefix,
        envPath,
      };

      if (!isJson) {
        console.log(chalk.green(`✓ Created SDK key and saved to ${TRAFFICAL_DIR}/.env`));
        console.log(chalk.dim(`  Key: ${newKey.apiKey.keyPrefix}... (sdk:read, sdk:write)`));
      }
    } catch (err) {
      if (!isJson) {
        console.log(chalk.yellow(`⚠ Could not create SDK key: ${err instanceof Error ? err.message : String(err)}`));
        console.log(chalk.dim(`  You can create one manually at https://app.traffical.io/settings/api-keys`));
      }
    }
  }
  ...
  if (isJson) {
    ...
    if (sdkKeyResult.created) {
      console.log(`  4. Add ${chalk.cyan("TRAFFICAL_API_KEY")} from ${TRAFFICAL_DIR}/.env to your project's .env`);
      console.log(`  5. Run ${chalk.cyan("traffical status")} to check sync status`);
    } else {
